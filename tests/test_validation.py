...
def test_compute_pathway_rate_ratios_basic(tmp_path):
    ...
    result = compute_pathway_rate_ratios(
        trans_df, area_df, transitions_csv, px_area_ha=1.0, n_timesteps=2
    )
    assert len(result) == 1
...
def test_compute_pathway_rate_ratios_unit_mismatch_fix(tmp_path):
    """
    Regression test for the v3.14 unit-mismatch fix, retained in v3.16
    despite the group-level revert: area_df's pool is in hectares, but
    the numerator (cells transitioned) is always a pixel count. Without
    converting the pool to pixels via px_area_ha first, simulated_rate
    (and therefore ratio) comes out off by a constant factor of
    1/px_area_ha. This test uses px_area_ha=0.09 (e.g. 30m pixels) and
    checks the rate reflects the pixel-converted pool, not the raw
    hectare figure.
    """
    transitions_csv = tmp_path / "Transitions.csv"
    transitions_csv.write_text(
    ...
    px_area_ha = 0.09  # e.g. 30m x 30m pixels
    result = compute_pathway_rate_ratios(
        trans_df, area_df, transitions_csv, px_area_ha=px_area_ha, n_timesteps=1
    )
    row = result.iloc[0]

    # Pool in pixels = 90.0 ha / 0.09 ha/px = 1000 px; 1 cell transitioned.
    expected_rate = 1 / (90.0 / px_area_ha)
    assert row["simulated_rate"] == pytest.approx(expected_rate)

    # The pre-fix (hectare-denominated) rate would have been 1/90, a much
    # larger and wrong value -- confirm the fixed result is NOT that.
    flawed_rate = 1 / 90.0
    assert row["simulated_rate"] != pytest.approx(flawed_rate)
    assert row["simulated_rate"] < flawed_rate
